...
local tooltip = CreateFrame("GameTooltip", "BONKTooltip", UIParent, "GameTooltipTemplate")
local function GetSpellTooltipText(spellID)
	tooltip:SetOwner(UIParent, "ANCHOR_NONE")
	tooltip:SetSpellByID(spellID)
	local lines = tooltip:NumLines()
	if lines < 1 then return end
	local line = _G["BONKTooltipTextLeft"..lines]:GetText()
...
function BCD:Initialize(BONK, db)
    self.BONK = BONK
    self.db = db
end

function BCD:GetSpells(key)
    local db = self.db
	local spells = {
		uncheck = {
			name = "Uncheck All",
			type = "execute",
			func = function(info)
				for option,_ in pairs(db[key].Track) do
					if option:match("^spell") then
						db[key].Track[option] = false
					end
				end
			end,
			order = 1,
		...
		for spellID, spell in pairs(self.cooldowns) do
			...
			if spell.class and spell.class == CLASS_SORT_ORDER[i] then
				...
				local extra = "\n\n|cffffd700 ".."Spell ID".."|r "..spellID..
					"\n\n|cffffd700 ".."Cooldown".."|r "..SecondsToTime(duration)
				...
				spells[CLASS_SORT_ORDER[i]].args["spell"..spellID] = {
                    ...
                    get = function(info)
                        ...
                        return db[key].Track[option]
                    end,
                    set = function(info, state)
                        local option = info[#info]
                        db[key].Track[option] = state
                    end
				}
...
function BCD:GetSpellDefaults()
    local defaults = {}

    for spellID, spell in pairs(self.cooldowns) do
        if spell.default then
            defaults["spell"..spellID] = true
        else
            defaults["spell"..spellID] = false
        end
    end
